...
const steps = [
  ...
  },
  {
    ...
    description:
      "You see real, working software every week—not slide decks. For qualifying projects, an agreed first milestone can be on us.",
  },
  {
    number: "03",
    title: "Launch & grow",
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {steps.map((step, i) => (
          <Reveal key={step.number} delay={i * 0.08}>
            <article className="card-soft card-lift relative h-full overflow-hidden rounded-3xl p-7">
              <span className="font-display text-5xl font-semibold tracking-tight text-primary/20 md:text-6xl">
                {step.number}
              </span>
              ...
              {i < steps.length - 1 && (
                <span
                  className="absolute -right-2 top-8 hidden text-2xl text-primary/25 md:block"
                  aria-hidden
                >
                  →
                </span>
