...
SQL migrations: backend/migrations, including durable scrape/OCR/outreach workflow, restaurant-media, demo-ready backfill, template-removal, OCR-reset, and a local unreleased demo-ready OCR/email gate migration through 000036. Deployed production schema is 000035. Integration test slot: backend/tests.
...
Local unreleased migration 000036 changes `demo_ready` to require `ocr_status='verified'` plus a nonblank restaurant email. Read-only production diagnosis before deployment showed verified=0, pending=940, failed=4, demo_ready=22, and demo_ready eligible under the new rule=0; OCR worker logs showed the 2026-07-19 UTC budget still exhausted at 200/200.
```
...
```text
2026-07-19 — Backend/Test/Documentation — Locally implemented unreleased migration 000036 and code changes so `demo_ready` is derived from verified OCR plus a nonblank email. Manual demo creation no longer marks demo-ready; Go restaurant edits and Python OCR/import finalization sync only lead/demo_ready rows from the new rule. Live read-only diagnosis showed zero OCR-verified production profiles, explaining the empty admin OCR verified filter, and OCR remains capped at 200/200 for 2026-07-19 UTC.
...
- Local unreleased change: `demo_ready` should be synchronized from verified OCR plus nonblank restaurant email, not from demo draft creation. Later lifecycle states remain operator/workflow-owned.
