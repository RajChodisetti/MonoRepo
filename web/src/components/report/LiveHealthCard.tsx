...
import { formatMetricScore } from "@/lib/report";
    ...
    <>
      ...
      <div className="rounded-[18px] bg-white px-3.5 pb-3.5 pt-4 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
        ...
        <ul className="mt-4 space-y-3">
          {metrics.map((metric) => (
            <li key={metric.key || metric.label} className="flex items-center gap-2.5">
              <div className="relative flex h-6 w-6 shrink-0 items-center justify-center">
                <ProgressRing value={metric.value} size={24} strokeWidth={3} color={metric.statusColor} />
              </div>
              <div className="min-w-0 flex-1 text-left">
                <p className="truncate text-[12.5px] font-semibold leading-tight text-[#111111]">
                  {metric.label}
                </p>
