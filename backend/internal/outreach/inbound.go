...
func (service *InboundService) capture(ctx context.Context, mailboxKey, gmailMessageID string) error {
	if _, err := service.store.GetMessageByGmailID(ctx, gmailMessageID); err == nil {
		return nil
...
func (service *InboundService) recentMailQuery() string {
	...
	// Inbox is the production path (dedicated outreach mailbox). Sent Re: covers
	// local same-account tests where Reply-To is a plus-address the sender does
	// not receive in Inbox.
	return fmt.Sprintf(
		`newer_than:7d (to:%s@%s OR subject:("Re: A live demo for Demo Fixture Cafe"))`,
		localPart,
		domain,
	)
}
