...
func NewGoogle(calendarID, credentialsPath string) (*Google, error) {
	...
	creds, err := google.CredentialsFromJSON(ctx, data, googlecalendar.CalendarScope)
	if err != nil {
		return nil, fmt.Errorf("parse service account credentials: %w", err)
	}
	service, err := googlecalendar.NewService(ctx, option.WithCredentials(creds))
	if err != nil {
...
func EnsureWritable(ctx context.Context, calendarID, credentialsPath string) (*Google, string, error) {
	...
	created, err := g.service.Calendars.Insert(&googlecalendar.Calendar{
	...
	if err != nil {
		...
		if pingErr := fallback.pingWrite(ctx); pingErr != nil {
			return nil, "", fmt.Errorf("calendar %q not writable and fallback failed: %w", calendarID, pingErr)
		}
...
func NewFromService(service *googlecalendar.Service, calendarID string) *Google {
...
func (g *Google) FreeBusy(ctx context.Context, start, end time.Time) ([]BusyPeriod, error) {
	req := &googlecalendar.FreeBusyRequest{
		TimeMin: start.UTC().Format(time.RFC3339),
		TimeMax: end.UTC().Format(time.RFC3339),
		Items:   []*googlecalendar.FreeBusyRequestItem{{Id: g.calendarID}},
	}
