...
func TestStartAdminPreviewRecordsSelectedTemplate(t *testing.T) {
	...
	result, err := service.StartAdminPreview(context.Background(), restaurantID, "2")
	if err != nil {
	...
	if !repo.created || repo.demoSiteID != nil || repo.restaurantID != restaurantID || repo.templateID != "2" {
		t.Fatalf("created session = %+v, want restaurant admin Aurora preview", repo)
	}
}
