...
func TestGetPlaceDetailsRetainsPrimaryTypeAndGooglePhotoAttribution(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for _, field := range []string{"primaryType", "photos", "googleMapsUri"} {
		...
		_, _ = w.Write([]byte(`{
			...
			"photos":[{
				...
				"authorAttributions":[{"displayName":"Google contributor"}]
			}]
		}`))
	}))
	defer server.Close()
	...
	if photo.Attribution != "Google contributor" || photo.GoogleMapsURI != "https://maps.google.com/photo/one" {
...
func TestSearchNearbyCuisineUsesTenKilometrePopularityAndFiltersResults(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"places":[
			...
			{"id":"near","displayName":{"text":"Nearby Thai"},"primaryType":"thai_restaurant","types":["thai_restaurant","restaurant"],"businessStatus":"OPERATIONAL","location":{"latitude":-33.85,"longitude":151.20}},
