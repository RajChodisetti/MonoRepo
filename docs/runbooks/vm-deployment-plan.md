...
`170.64.154.143`, using root `web/` as the canonical corporate website.
...
| Tuvi corporate website | `web/` | Next.js app | Canonical public website, containerized on VM loopback |
...
Build root `web/` with `infra/docker/Dockerfile.tuvi-website` and serve
...
7. `admin-web`, built from `apps/web/` and path-routed by Caddy at `/admin`.
8. `voice-agent`, built from `voice-sales-agent/`.
9. Only loopback host ports; public traffic stays on host-level Caddy.
...
1. Code checkout
   - Push the reviewed deployment commit without merging it to `main` unless
     that separate approval has been granted.
   - Extract that exact commit into `/opt/tuvi/releases/monorepo-<short-sha>`;
     never deploy a moving branch tip.
   - Keep `/opt/tuvi/MonoRepo` as a symlink to the active immutable release and
     record the previous release path before switching it.
...
2. Secret setup
   ...
   - Ensure `TOKEN_SECRET`, `TUVI_API_TOKEN`, and every secret required by the
     enabled inbound providers are production values.
...
3. Quiesce, migrate, and start the updated VM stack
   ...
   - Before migration `000041`, run the following read-only preflight and stop if
     either count is non-zero. Also compare every stored migration version/name
     with the checked-out filenames; the migration runner now fails closed on a
     lineage mismatch.

     ```sql
     SELECT count(*) AS invalid_consultation_intervals
     FROM company_consultations
     WHERE slot_end <= slot_start;

     SELECT count(*) AS overlapping_confirmed_consultations
     FROM company_consultations AS first_call
     JOIN company_consultations AS second_call
       ON first_call.id < second_call.id
      AND first_call.status = 'confirmed'
      AND second_call.status = 'confirmed'
      AND tstzrange(first_call.slot_start, first_call.slot_end, '[)')
          && tstzrange(second_call.slot_start, second_call.slot_end, '[)');
   ...
   - After the updated core processes are running, build/start `admin-web`, the
     corporate website, and voice service. Rebuild the catalog or restaurant
     template only when that release changes them; these services do not replace
     the controlled migration sequence.
   - Confirm API, Go worker, scrape worker, Postgres, Redis, corporate website,
     admin web, catalog, template, and voice containers are running.
   - Smoke `https://api.tuvisolutions.com/admin/login`, authenticate as an
     internal admin, and read the current consultation month before considering
     the calendar UI deployed. Do not write production availability as a smoke
     test.
...
9. Smoke checks
    ...
    - `https://tuvisolutions.com/book`
    - `https://tuvisolutions.com/services/restaurants` permanently redirects to `/`.
    - `https://api.tuvisolutions.com/api/public/v1/site/restaurants`
    - `https://api.tuvisolutions.com/admin/login`
    - `https://voice.tuvisolutions.com/readyz/browser`
    - `https://demo.tuvisolutions.com`
    - Read-only Tuvi booking availability and slot-check requests through the
      website/API. Do not create a fake production booking as a smoke test.
    - A browser-originated inbound voice session that asks for availability but
      does not submit booking details.
    - If separately approved, an inbound Twilio call to `/twiml`; never place an
      outbound AI callback as a deployment smoke test.
...
1. Keep the previous immutable release directory and record the active commit before changing the `/opt/tuvi/MonoRepo` symlink.
2. Before deploy, take a Postgres backup.
3. Treat migrations `000039` through `000041` as forward-only for this rollout.
   Do not run their down migrations: `000039` drops saved availability,
   `000040` cannot reconstruct template-4 identities, and `000041` removes the
   confirmed-call overlap constraint and the month revisions required by the
   calendar-aware API.
4. An API image from before the consultation-calendar release is not booking-
   compatible after slot overrides exist: it does not read those overrides or
   take the month/slot locks. Before switching to any such image, disable both
   consultation API route families, the corporate booking entry points, and the
   voice agent's consultation tools. Keep that traffic disabled for the entire
   emergency rollback; do not restart the pre-calendar API for live bookings
   merely because the newer schema is still present.
5. Prefer a forward fix on a calendar-aware API image. If a database rollback is
   unavoidable, keep consultation traffic stopped and restore the validated
   pre-deploy backup under explicit data-recovery approval, then start only the
   application release compatible with that restored database.
...
docker compose --env-file /opt/tuvi/env/stack.env -p tuvi -f infra/docker/docker-compose.vm.yml logs -f --tail=200 api worker scrape-worker voice-agent tuvi-website admin-web restaurant-services-catalog template
docker stats
df -h
```
...
- Outbound AI calls are hard-disabled in Phase 1. Do not add callback tools,
  dialing routes, or provider calls without a separately approved policy change.
...
5. Enable email only after the Gmail API/OAuth mailbox credentials, canonical
   HTTPS links, and the audited human approval flow are reviewed and configured.
   Consultation availability remains PostgreSQL-only; any future external
   calendar integration requires a separate approved design for reconciliation
